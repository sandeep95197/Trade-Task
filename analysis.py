"""
ai_analyzer.py – Generates structured Markdown trade reports using Google Gemini.
Falls back to a well-structured static template if the API key is absent.
"""
    ...
    async def generate_report(self, sector: str, market_data: dict) -> str:
        if GEMINI_API_KEY:
            try:
                ...
                logger.error("Gemini API error: %s – falling back to template", exc)
...
> *Generated: {datetime.utcnow().strftime('%B %d, %Y')} | Source: AI-Powered Market Intelligence*
...
---
*This report is AI-generated for informational purposes. Verify data before making investment decisions.*
"""
    ...
    def _template_report(self, sector: str, market_data: dict) -> str:
        news_block = "\n".join(
            ...
            for item in market_data.get("news", [])
            if item.get("title")
        ) or "- Live news not available; configure GEMINI_API_KEY for enriched reports."
...
India's **{sector.title()}** sector represents a significant trade opportunity in the current global
landscape. With favourable government policies, growing domestic capacity, and rising international
demand, the sector is positioned for accelerated export growth through 2026-2030.
...
- 📌 **Government Export Incentives** — RoDTEP, MEIS successor schemes
- 📌 **Growing Domestic R&D** — Rising patent filings & innovation output
...
| Import Dependency | Declining | ↓ Good |
...
3. **Attend India International Trade Fair (IITF)** & sector-specific expos
...
---
*⚠️ This report is AI-generated for informational purposes only. Always verify data with official sources before making investment or trade decisions.*
"""
